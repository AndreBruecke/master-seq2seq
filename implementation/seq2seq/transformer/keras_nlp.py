...
def prepare_asymmetric_batches(df: pd.DataFrame, encoder_dce, decoder_dce, num_samples: int, validation_split: float, batch_size: int, random_state: int):
    def prepare_batch_char(input, target):
        input = input.to_tensor()
        target_inputs = target[:, :-1].to_tensor()  # Drop end boundary (\n)
        target_labels = target[:, 1:].to_tensor()   # Drop start boundary (\t)
    ...
    # Label encoding
    train_input_ids = encoder_dce.to_ids(train_input, insert_markers=True)
    train_target_ids = decoder_dce.to_ids(train_target, insert_markers=True)
    val_input_ids = encoder_dce.to_ids(val_input, insert_markers=True)
    val_target_ids = decoder_dce.to_ids(val_target, insert_markers=True)
...
def construct_asymmetric_model_w_teacher_forcing(num_encoder_layers: int, num_decoder_layers: int, encoder_unique_tokens: int, decoder_unique_tokens: int, max_encoder_seq_length: int, max_decoder_seq_length: int, embedding_dim: int, intermediate_dim: int, encoder_heads: int, decoder_heads: int, dropout: float):
    encoder_inputs = tf.keras.Input(shape=(max_encoder_seq_length+2,), name='encoder_inputs')
    token_embeddings = tf.keras.layers.Embedding(input_dim=encoder_unique_tokens, output_dim=embedding_dim)(encoder_inputs)
    position_embeddings = keras_nlp.layers.PositionEmbedding(sequence_length=max_encoder_seq_length+2)(token_embeddings)
    encoder_outputs = token_embeddings + position_embeddings
    ...
    token_embeddings = tf.keras.layers.Embedding(input_dim=decoder_unique_tokens, output_dim=embedding_dim)(decoder_inputs)
    position_embeddings = keras_nlp.layers.PositionEmbedding(sequence_length=max_decoder_seq_length+2)(token_embeddings)
    decoder_outputs = token_embeddings + position_embeddings
    ...
    # decoder_outputs = tf.keras.layers.Dropout(0.25)(decoder_outputs)
    decoder_outputs = tf.keras.layers.Dense(decoder_unique_tokens, activation="softmax")(decoder_outputs)
