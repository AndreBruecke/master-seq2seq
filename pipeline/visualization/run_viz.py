...
for input in [["WIKIDATA_ORG", WIKIDATA_ORG_QUERY_RESULT], ["WIKIDATA_LOC", WIKIDATA_LOC_QUERY_RESULT], ["WIKIDATA_HUMAN", WIKIDATA_HUMAN_QUERY_RESULT]]:
    ...
    df['count'] = 1
    variants_per_entity = df[['id', 'label', 'count']].drop_duplicates()[['id', 'count']].groupby(by='id', as_index=False)['count'].count()
...
df['count'] = 1
variants_per_entity = df[['geonameid', 'name', 'count']].drop_duplicates()[['geonameid', 'count']].groupby(by='geonameid', as_index=False)['count'].count()
